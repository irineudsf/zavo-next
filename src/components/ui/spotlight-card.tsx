...
const glowColorMap = {
  ...
  amber:  { base: 45,  spread: 0   }, // âmbar fixo — não muda com o mouse
}
...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const getInlineStyles = (): React.CSSProperties & Record<string, string | number> => {
    const styles: React.CSSProperties & Record<string, string | number> = {
      ...
      '--radius': '14',
      '--border': '2',
      '--backdrop': 'hsl(0 0% 95% / 1)',
      '--backup-border': 'hsl(45 0% 88% / 1)',
      '--size': '280',
      '--outer': '1',
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 45) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 60) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
      )`,
      backgroundColor: 'var(--backdrop, #F5F5F5)',
  ...
  const css = `
    ...
    [data-glow]::before {
      background-image: radial-gradient(
        ...
        hsl(var(--hue, 45) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / var(--border-spot-opacity, 0.9)), transparent 100%
      );
    ...
    <>
      ...
      <div
        ...
        className={`
          ...
          rounded-2xl relative grid shadow-[0_1rem_2rem_-1rem_rgba(0,0,0,0.08)] p-0 backdrop-blur-[2px]
          ${className}
        `}
      >
